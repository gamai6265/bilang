...
public class UniWebView : MonoBehaviour {
	...
	[SerializeField]
	private UniWebViewEdgeInsets _insets = new UniWebViewEdgeInsets(100,50,100,50);
	...
	private void LoadComplete(string message) {
		...
		if (loadSuc) {
            ...
            LogSystem.Warn("Web page load failed:{0}; url:{1}; error:{2}",gameObject.name,url,message);
			if (hasCompleteListener) {
	...
	private void LoadBegin(string url) {
        LogSystem.Debug("Begin to load: " + url);
		if (OnLoadBegin != null) {
